...
import { Supplier, NewSupplier, UpdateSupplier, ExpenseCategory, getCategoryKeyByName } from '@/types/supplier';
...
export async function getSuppliersByCategory(category: ExpenseCategory): Promise<Supplier[]> {
  try {
    const { data, error } = await supabase
      .from('suppliers')
      .select('*')
...
function getCategoryByKeywords(supplierName: string): ExpenseCategory {
  ...
  const knownCompanies: Record<string, ExpenseCategory> = {
    'алрус': 'profile',
    'металлмастер': 'profile', 
    'металлмастер-м': 'profile',
    'озеров максим николаевич': 'delivery',
    'озеров': 'delivery',
    'эксперт рентал инжиниринг': 'lifting_equipment',
    'ал-профи': 'profile',
    'практик': 'fittings',
    'петрович': 'hardware',
  ...
  const categoryKeywords: Record<ExpenseCategory, string[]> = {
    profile: ['профиль', 'алюминий', 'пвх', 'окна', 'рама'],
    components: ['комплектующие', 'запчасти', 'детали'],
    ...
    delivery: ['доставка', 'логистика', 'транспорт', 'перевозка'],
    lifting_equipment: ['подъем', 'кран', 'лебедка', 'подъемник', 'рентал', 'аренда', 'инжиниринг', 'экспер'],
    brackets: ['кронштейн', 'крепеж', 'опора'],
    hardware: ['метиз', 'болт', 'гайка', 'шуруп', 'саморез'],
    adjoining: ['примыкание', 'герметик', 'уплотнитель'],
    additional: ['дополнительно', 'прочее', 'разное'],
    custom: []
  };
  ...
  console.log(`[CATEGORY-KEYWORDS] ❌ Категория не определена, используем 'additional'`);
  return 'additional'; // По умолчанию
}