...
import { invoiceFileService } from '@/services/invoice-files';
...
// Инициализация Supabase клиента для записи в БД
const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL || 'http://127.0.0.1:54321',
  process.env.SUPABASE_SERVICE_ROLE_KEY || 'sb_secret_N7UND0UgjKTVK-Uodkm0Hg_xSvEMPvz'
);

// Функция для получения или создания поставщика
// Возвращает категорию поставщика из БД или 'additional' для нового
async function getOrCreateSupplier(name: string, inn: string): Promise<{ id: string | null; category: ExpenseCategory }> {
  if (!name || name === 'Неизвестный поставщик') {
    return { id: null, category: 'additional' };
  }
  
  try {
    console.log(`🏢 [SMART-INVOICE] Проверяем поставщика: ${name} (ИНН: ${inn})`);
    
    // Сначала ищем по ИНН (если указан)
    ...
    // Если не нашли по ИНН, ищем по имени
    if (!existingSupplier) {
      const { data, error } = await supabase
        .from('suppliers')
        .select('*')
    ...
    if (existingSupplier) {
      console.log(`✅ [SMART-INVOICE] Найден существующий поставщик: ${existingSupplier.id} (категория: ${existingSupplier.category})`);
      return { id: existingSupplier.id, category: existingSupplier.category as ExpenseCategory };
    }
    
    // Создаем нового поставщика с категорией по умолчанию
    const { data: newSupplier, error: createError } = await supabase
      .from('suppliers')
      .insert({
        name,
        inn: inn || null,
        category: 'additional', // Категория по умолчанию - пользователь установит вручную
        description: `Автоматически создан при обработке счета`,
        is_active: true
      })
      .select()
      .single();
    ...
    if (createError) {
      console.error('❌ [SMART-INVOICE] Ошибка создания поставщика:', createError);
      return { id: null, category: 'additional' };
    } else {
      console.log(`✅ [SMART-INVOICE] Создан новый поставщик: ${newSupplier.id} (${name}) с категорией "additional"`);
      return { id: newSupplier.id, category: 'additional' };
    }
  } catch (error) {
    console.error('❌ [SMART-INVOICE] Ошибка в getOrCreateSupplier:', error);
    return { id: null, category: 'additional' };
  }
}
...
export async function POST(request: NextRequest) {
  ...
  console.log(`📨 [SMART-INVOICE-${requestId}] Получен новый запрос`);
  ...
  try {
    ...
    if (!projectId) {
      return NextResponse.json({ error: 'ID проекта не найден' }, { status: 400 });
    }
    
    console.log(`📄 [SMART-INVOICE-${requestId}] Файл: ${file.name}, Размер: ${file.size} байт, Тип: ${file.type}`);
    
    // Проверяем тип файла
    const fileExtension = file.name.toLowerCase().split('.').pop();
    const supportedTypes = ['pdf', 'xlsx', 'xls', 'docx', 'doc', 'txt', 'png', 'jpg', 'jpeg', 'webp', 'heic'];
    ...
    // Загружаем файл в Supabase Storage для долгосрочного хранения
    let cloudFileUrl = '';
    let cloudFilePath = '';
    let useLocalFallback = false;
    
    try {
      console.log(`☁️ [SMART-INVOICE] Загружаю файл в облачное хранилище...`);
      const cloudUpload = await invoiceFileService.uploadInvoiceFile(file);
      
      if (cloudUpload && cloudUpload.url) {
        // Если URL начинается с http/https - это облачная ссылка Supabase, используем как есть
        // Если URL начинается с /api/files/ - это локальный файл, используем как есть
        // Иначе добавляем префикс /api/files/
        if (cloudUpload.url.startsWith('http://') || cloudUpload.url.startsWith('https://')) {
          cloudFileUrl = cloudUpload.url; // Полный URL от Supabase
        } else if (cloudUpload.url.startsWith('/api/files/')) {
          cloudFileUrl = cloudUpload.url; // Уже локальный путь
        } else {
          cloudFileUrl = `/api/files/${cloudUpload.url}`; // Добавляем префикс
        }
        cloudFilePath = cloudUpload.path;
        console.log(`✅ [SMART-INVOICE] Файл загружен в облако: ${cloudFileUrl}`);
        console.log(`📂 [SMART-INVOICE] Путь в хранилище: ${cloudFilePath}`);
      } else {
        console.log(`⚠️ [SMART-INVOICE] Не удалось загрузить в облако, используем локальный путь`);
        useLocalFallback = true;
        cloudFileUrl = `/api/files/${tempFileName}`; // Локальная ссылка через API
        cloudFilePath = tempFileName;
      }
    } catch (uploadError) {
      console.error(`❌ [SMART-INVOICE] Ошибка загрузки в облако:`, uploadError);
      useLocalFallback = true;
      cloudFileUrl = `/api/files/${tempFileName}`; // Локальная ссылка через API
      cloudFilePath = tempFileName;
    }
    ...
    if (['xlsx', 'xls', 'docx', 'doc'].includes(fileExtension)) {
      ...
      try {
        ...
        console.log(`✅ [SMART-INVOICE-${requestId}] Текст извлечен: ${textResult.text_length} символов`);
      } catch (parseError) {
    ...
    } else if (['png', 'jpg', 'jpeg', 'gif', 'bmp', 'tiff', 'webp', 'heic'].includes(fileExtension)) {
      // Обработка изображений напрямую через OCR
      console.log(`🖼️ [SMART-INVOICE] Обработка изображения...`);
      ...
      try {
        ...
        [docResult] = await vision.documentTextDetection({
          image: { content: imageBuffer },
        });
        
        console.log(`📋 [SMART-INVOICE] Используем textDetection...`);
        [textResult] = await vision.textDetection({
          image: { content: imageBuffer },
        });
        
        const docText = docResult.fullTextAnnotation?.text || '';
        const simpleText = textResult.textAnnotations?.[0]?.description || '';
        ...
        console.log(`📝 [SMART-INVOICE] Текст изображения (первые 200 символов): ${pageText.substring(0, 200).replace(/\n/g, ' ')}`);
        
      } catch (visionError) {
        console.error(`❌ [SMART-INVOICE] Ошибка Google Vision API:`, visionError);
        throw new Error(`Ошибка обработки изображения: ${visionError}`);
      }
      ...
      for (const image of pdfData.images) {
        ...
        try {
          ...
          console.error(`🚨 [SMART-INVOICE] Ошибка Google Vision API:`, visionError?.message);
          
          // Проверяем если это ошибка аутентификации
          if (visionError?.message?.includes('UNAUTHENTICATED') || visionError?.message?.includes('authentication')) {
            console.log(`🔐 [SMART-INVOICE] Google Vision API временно недоступен, используем демо данные`);
            
            // Создаем демо данные для демонстрации функционала
            const testText = generateTestInvoiceText();
            docResult = {
              fullTextAnnotation: { text: testText },
              textAnnotations: [{ description: testText, score: 0.8 }]
            };
            textResult = {
              textAnnotations: [{ description: testText, score: 0.8 }]
            };
          } else {
            throw visionError; // Перебрасываем другие ошибки
          }
        }
    ...
    try {
      ...
      // Добавляем детальное логирование полученных данных
      console.log('🔍 [SMART-INVOICE] Полученные данные от парсера:', JSON.stringify(parsedData, null, 2));
      console.log('🏢 [SMART-INVOICE] Название поставщика из парсера:', parsedData.contractor?.name);
      console.log('🏷️ [SMART-INVOICE] ИНН поставщика из парсера:', parsedData.contractor?.inn);
      console.log('🏷️ [SMART-INVOICE] Тип contractor:', typeof parsedData.contractor);
      console.log('🏷️ [SMART-INVOICE] Contractor keys:', parsedData.contractor ? Object.keys(parsedData.contractor) : 'нет');
    ...
    try {
      console.log('💾 [SMART-INVOICE] Сохранение счета в базу данных...');
      
      // Определяем категорию на основе поставщика
      const supplierName = parsedData.contractor?.name || 'Неизвестный поставщик';
      const supplierInn = parsedData.contractor?.inn || '';
      
      console.log('🔍 [SMART-INVOICE] Перед определением категории:');
      console.log('  - supplierName:', supplierName);
      console.log('  - supplierInn:', supplierInn);
      console.log('  - parsedData.contractor:', JSON.stringify(parsedData.contractor, null, 2));
      
      // Получаем или создаём поставщика и берём его категорию из БД
      const supplierData = await getOrCreateSupplier(supplierName, supplierInn);
      const categoryKey = supplierData.category;
      const supplierId = supplierData.id;
      const categoryName = expenseCategoryMap[categoryKey] || 'Доп. затраты';
      
      console.log('🏷️ [SMART-INVOICE] Определена категория из БД:', {
        supplierName,
        supplierId,
        categoryKey,
        categoryName
      ...
      const invoiceData = {
        ...
        invoice_number: parsedData.invoice?.number || 'Без номера',
        issue_date: parsedData.invoice?.date || new Date().toISOString().split('T')[0],
        total_amount: parsedData.invoice?.total_amount || 0,
        vat_amount: parsedData.invoice?.vat_amount || 0,
        has_vat: parsedData.invoice?.has_vat || false,
        payment_status: 'pending',
        category: categoryKey,
        file_url: cloudFileUrl,
        original_file_name: cloudFilePath,
      };
      
      console.log('📄 [SMART-INVOICE] Данные для сохранения в БД:', JSON.stringify(invoiceData, null, 2));
      
      // Проверяем на дубли по номеру счета и поставщику
      const { data: existingInvoices, error: searchError } = await supabase
        .from('invoices')
        .select('*')
        .eq('project_id', projectId)
        .eq('invoice_number', invoiceData.invoice_number)
        .eq('supplier_name', invoiceData.supplier_name);
        
      if (searchError) {
        console.error('❌ [SMART-INVOICE] Ошибка поиска дублей:', searchError);
      }
      
      if (existingInvoices && existingInvoices.length > 0) {
        const existingInvoice = existingInvoices[0];
        console.log('🔍 [SMART-INVOICE] Найден существующий счет:', existingInvoice.id);
        
        // Если сумма отличается, обновляем
        if (existingInvoice.total_amount !== invoiceData.total_amount) {
          console.log(`💰 [SMART-INVOICE] Обновляем сумму с ${existingInvoice.total_amount} на ${invoiceData.total_amount}`);
          ...
          const { data: updatedInvoice, error: updateError } = await supabase
            ...
            .update({
              ...
              file_url: cloudFileUrl, // Обновляем ссылку на файл в облаке
              original_file_name: cloudFilePath, // Обновляем путь к файлу
          ...
          if (updateError) {
            console.error('❌ [SMART-INVOICE] Ошибка обновления счета:', updateError);
          } else {
            console.log('✅ [SMART-INVOICE] Счет обновлен с ID:', updatedInvoice.id);
            savedInvoice = updatedInvoice;
          }
        } else {
          console.log('ℹ️ [SMART-INVOICE] Счет уже существует с той же суммой, пропускаем');
        ...
        if (error) {
          console.error('❌ [SMART-INVOICE] Ошибка сохранения в БД:', error);
          // Не прерываем выполнение, просто логируем ошибку
        } else {
          console.log('✅ [SMART-INVOICE] Счет сохранен в БД с ID:', invoice.id);
          savedInvoice = invoice;
        }
      }
    } catch (dbError: any) {
      console.error('❌ [SMART-INVOICE] Ошибка работы с БД:', dbError);
      // Не прерываем выполнение, просто логируем ошибку
    }
    
    // Очистка временных файлов (оставляем файл для возможности просмотра)
    // try {
...
function generateTestInvoiceText(): string {
  const invoiceNumber = `ТЕС-${Math.floor(Math.random() * 10000)}`;
  const currentDate = new Date().toLocaleDateString('ru-RU');
  const amount = (Math.random() * 50000 + 10000).toFixed(2);
  const tax = (parseFloat(amount) * 0.2).toFixed(2);
...
Товары и услуги:
1. Тестовая услуга - ${amount} руб.

Сумма без НДС: ${amount} руб.
НДС 20%: ${tax} руб.
Всего к оплате: ${total} руб.

Банковские реквизиты:
Р/с: 40702810012345678901
Банк: ПАО "Тестовый банк"
